...
class CaptioningModel(nn.Module):
    ...
    def forward(self, enc_x, dec_x=None,
        ...
        if mode == 'forward':
            ...
            sugg_input = self.forward_bidir(x, enc_x_num_pads, sugg_input, sugg_num_pads)
            y = self.forward_dec(x, enc_x_num_pads, dec_x, dec_x_num_pads,
                                 sugg_input, sugg_num_pads,
                                 apply_log_softmax)
            return y
        else:
            ...
            if mode == 'sampling':
                ...
                out_classes, out_logprobs = self.get_batch_multiple_sampled_prediction(
                    ...
                    num_outputs=how_many_outputs,
                    sos_idx=sos_idx, eos_idx=eos_idx,
                    max_seq_len=sample_max_seq_len)
                return out_classes, out_logprobs

    def get_batch_multiple_sampled_prediction(self, enc_input, enc_input_num_pads,
        ...
        sugg_input = self.forward_bidir(x, enc_input_num_pads, sugg_input, sugg_num_pads)

        sugg_num_pads = [sugg_num_pads[i] for i in range(bs) for _ in range(num_outputs)]
        sugg_input = sugg_input.unsqueeze(1).expand(-1, num_outputs, -1).reshape(
            bs * num_outputs, sugg_input.size(1))
        ...
        while (finished_flag_vector.sum() != bs * num_outputs) and time_step < max_seq_len:
            ...
            log_probs = self.forward_dec(x, enc_input_num_pads, dec_input, dec_input_num_pads,
                                         sugg_input=sugg_input,
                                         sugg_num_pads=sugg_num_pads,
    ...
    def beam_search(self, enc_input, enc_input_num_pads,
        ...
        #print("@@@@@@@@@@@@@@@@@@@---------")
        #print("sugg_input: FFFFFFF " + str(sugg_input.shape))

        cross_enc_output = self.forward_enc(enc_input, enc_input_num_pads)

        bs, num_ngrams, k_gram = sugg_input.shape
        #UNPACKED_sugg_num_pads = [p*2 for p in sugg_num_pads]
        UNPACKED_sugg_num_pads = [p for p in sugg_num_pads]
        sugg_out = self.forward_bidir(cross_enc_output, enc_input_num_pads,
                                      sugg_input, UNPACKED_sugg_num_pads)
        #print("sugg_input: ZZZZZZZZZZZ " + str(sugg_out.shape))
        ...
        log_probs = self.forward_dec(cross_input=cross_enc_output, enc_input_num_pads=enc_input_num_pads,
                                     ...
                                     sugg_input=sugg_out, sugg_num_pads=sugg_num_pads,
                                     apply_log_softmax=True)
        ...
        #print("==== sugg: " + str(sugg_out.shape))
        bs, sugg_seq_len = sugg_out.shape
        sugg_out = sugg_out.unsqueeze(1)
        sugg_out = sugg_out.expand(bs, beam_size, d_model)
        sugg_out = sugg_out.reshape(bs * beam_size, d_model).contiguous()
        sugg_num_pads = [UNPACKED_sugg_num_pads[i] for i in range(bs) for _ in range(beam_size)]
        #print("sugg_@@@@: " + str(sugg_out.shape))
