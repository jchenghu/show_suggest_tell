...
def load_most_recent_checkpoint(model,
    ...
    for file_name in ls_files:
        if file_name.startswith(prefix + 'checkpoint_'):
            ...
            if (most_recent_checkpoint_datetime is None) or \
                ...
                most_recent_checkpoint_filename = prefix + file_name
...
def save_last_checkpoint(model,
    ...
    new_checkpoint_filename = prefix + 'checkpoint_' + datetime.now().strftime(datetime_format) + \
