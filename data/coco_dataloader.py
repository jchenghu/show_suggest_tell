...
class CocoDataLoader(TransparentDataLoader):
    ...
    def __init__(self, coco_dataset,
        ...
        self.discovered_words = list(set(self.coco_dataset.caption_word2idx_dict.keys()))  # per rimuovere pad eos repeatitions in giro...

        time_start = time()
        self.unique_words_for_each_image = []
        for img_idx in range(len(self.coco_dataset.karpathy_train_list)):
            captions_group = self.coco_dataset.karpathy_train_list[img_idx]['captions']

            unique_words_tokens = []
            for captions in captions_group:
                for caption in captions:
                    ...
                    unique_words_tokens += tokenized_caption
            unique_words_tokens = list(set(unique_words_tokens))

            discovered_words = [self.coco_dataset.caption_word2idx_dict[word] for word in unique_words_tokens
                                if word in self.discovered_words]
            self.unique_words_for_each_image.append(discovered_words)
            if (img_idx+1) % 20000 == 0:
                print("Creating n-grams progress: " + str(img_idx+1) + " / " + str(self.coco_dataset.train_num_images) + " elapsed: " + str(time()-time_start) + " seconds.")
    ...
    def get_next_batch(self, verbose=False, get_also_image_idxes=False, get_also_image_path=False):
        ...
        for i in range(len(img_idx_batch)):
            ...
            img_gt_group_ngrams.append(self.coco_dataset.karpathy_train_list[img_idx]['ngrams_imgwise_str'])

        batch_suggestions_len = []
        for ngram_set in img_gt_group_ngrams:
            fff = [self.coco_dataset.convert_string2listngram(ngram_str) \
                   for ngram_str in ngram_set]
            batch_suggestions_len.append(len(fff))
        max_num_ngrams = max(batch_suggestions_len)
        batch_suggestions_num_pads = []
        for num_ngrams in batch_suggestions_len:
            batch_suggestions_num_pads.append(max_num_ngrams - num_ngrams)
        ...
        for ngram_set in img_gt_group_ngrams:
            ...
            ngrams_tensor = []
            for f_f in fff:
                ngrams_tensor.append(torch.tensor([self.coco_dataset.caption_word2idx_dict[f] for f in f_f]))
            ngrams_tensor = torch.stack(ngrams_tensor)  # [ngrams, 2]

            # padding time
            if num_ngrams != max_num_ngrams:
                diff_len = max_num_ngrams - num_ngrams
                ngrams_tensor = torch.cat([ngrams_tensor,
                                torch.ones(diff_len, self.coco_dataset.k_gram, dtype=torch.long) * self.coco_dataset.caption_word2idx_dict['PAD']
                                ], dim=0)

            batch_suggestions.append(ngrams_tensor)
        batch_suggestions = torch.stack(batch_suggestions, dim=0)
        ...
        if get_also_image_idxes:
            if self.dataloader_mode == 'caption_wise':
                # # # # # # # # # # # # # # # # # # # # #
                #       Adding Suggestions Here
                # # # # # # # # # # # # # # # # # # # # #
                return batch_x, batch_y, batch_x_num_pads, batch_y_num_pads, \
                    batch_suggestions, batch_suggestions_num_pads, \
                    img_idx_batch
            else:
    ...
    def get_ngrams_by_idxes(self, img_idxes, dataset_split, get_also_id=False):
        img_gt_group_ngrams = []
        for img_idx in img_idxes:
            if dataset_split == CocoDatasetKarpathy.TestSet_ID:
                img_gt_group_ngrams.append(self.coco_dataset.karpathy_test_list[img_idx]['ngrams_imgwise_str'])
            elif dataset_split == CocoDatasetKarpathy.ValidationSet_ID:
                img_gt_group_ngrams.append(self.coco_dataset.karpathy_val_list[img_idx]['ngrams_imgwise_str'])
            else:
                img_gt_group_ngrams.append(self.coco_dataset.karpathy_train_list[img_idx]['ngrams_imgwise_str'])
        ...
        for ngram_set in img_gt_group_ngrams:
            ...
            if num_ngrams != max_num_ngrams:
                diff_len = max_num_ngrams - num_ngrams
                ngrams_tensor = torch.cat([ngrams_tensor,
                                           torch.ones(diff_len, self.coco_dataset.k_gram, dtype=torch.long) *
                                           self.coco_dataset.caption_word2idx_dict['PAD']
                                           ], dim=0)
        ...
        return batch_suggestions, batch_suggestions_num_pads
        ...
        for img_idx in img_idx_list:
            if self.coco_dataset.preproc_images_hdf5_filepath is not None:
                ...
                try:
                    ...
                    print("Failed image processing at img_path: " + str(file_path))
                    print("Maybe this was corrupted, try load this image again")
    ...
    def get_image_path_by_idx(self, img_idx, dataset_split):
        if dataset_split == CocoDatasetKarpathy.TestSet_ID:
    ...
    def get_images_by_idx(self, img_idx, dataset_split, transf_mode='train', get_also_id=False,
        ...
        if get_also_image_path:
            return tens_image, file_path

        return tens_image

    def get_sugg_by_idx(self, img_idx, dataset_split):
        if dataset_split == CocoDatasetKarpathy.TestSet_ID:
            img_id = self.coco_dataset.karpathy_test_list[img_idx]['img_id']
        elif dataset_split == CocoDatasetKarpathy.ValidationSet_ID:
            img_id = self.coco_dataset.karpathy_val_list[img_idx]['img_id']
        ...
        unique_words = self.coco_dataset.suggestion_json[str(img_id)].split(' ')
        # remove special tokens
        special_tokens = ['SOS', 'EOS', 'MASK', 'PAD', 'UNK']
        NEW_unique_words = []
        ...
        try:
            unique_words_tokens = [self.coco_dataset.caption_word2idx_dict[word] for word in unique_words]
            # sometimes the prediction would contain PAD, EOS, MASK, PAD, UNK...
            # like... too much stuff...
        except:
            print("Raised an Issue...")
            print(unique_words)
        unique_words_tokens = torch.tensor(unique_words_tokens)
        return unique_words_tokens
